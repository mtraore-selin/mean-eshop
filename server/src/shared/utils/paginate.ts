...
function paginateSchema(query, options: PaginateOptions): Promise<{ all: Product[]; pagination; maxPrice: number; minPrice: number }> {
  query = query || {};
  ...
  const all = limit ? this.find(query).sort(sort).skip(skip).limit(limit).exec() : query.exec();
